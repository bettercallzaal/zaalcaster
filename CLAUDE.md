...
- WEB roadmap (Zaal 2026-07-05, "main client he can push to vercel"): 1 foundation DONE (#19). 2 drafts DONE - api/draft.js + draft-all button, verified via mock handler (200, 2 real items drafted, guards 405/400, no leak). NEXT: 3 /api/send.js (reply with an explicit confirm step in the UI, never auto-post - the button press is Zaal's yes, show exact text first), 4 voice-example capture on web edits, 5 mobile polish + health/status indicator. STILL UNPROVEN across both CLI and web: a real send has never fired - first live post is the key validation.
- SPACES roadmap (Zaal 2026-07-05, integrate ZAODEVZ/Zuke + play computer audio into spaces): 1 DONE - juke.js + spaces command (create/info/embed). 2 DONE (Zaal chose audio-DJ first, delivered in our web page) - public/juke-dj.js (JukeDJ class: getUserMedia bridge, getDisplayMedia computer-audio capture, file playback, mic/music gain, level meter, MediaStreamDestination mix) + public/dj.html (DJ panel, Juke brand palette, linked from cockpit). Bridge logic unit-tested in Node with Web Audio stubs (10/10: patch/restore getUserMedia, mix folds real mic, gain clamps). NEXT: 3 wire the Juke SDK + LiveKit into dj.html so the mixed track actually PUBLISHES to a real space Zaal hosts (needs JUKE_API_KEY + host/speaker promotion + a real browser to test - can't verify headless); 4 embed a space + SIWF join in the cockpit. Honest caveat still stands: publishing is the unproven half; the mix+bridge core is built and tested, real audio capture + publish is Zaal's to drive in a browser.
