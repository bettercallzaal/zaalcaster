...
- SPACES roadmap (Zaal 2026-07-05, integrate ZAODEVZ/Zuke + play computer audio into spaces): 1 DONE - juke.js + spaces command (create/info/embed). Space DJ (Downloads/space-dj-1.0.2.zip) is the audio mechanism: a Chrome extension that monkey-patches getUserMedia + mixes captured tab audio into the mic via Web Audio, so LiveKit publishes music+voice. That trick is BROWSER-ONLY and needs host/speaker promotion in the space. NEXT: 2 get a JUKE_API_KEY (juke.audio/developers approval or reuse Zuke's) then create a real space; 3 web cockpit: embed a Juke space + SIWF join; 4 the DJ audio-bridge (getUserMedia interception + tab/system-audio mixing) so Zaal can play any computer audio into a space he hosts - build in the browser page (not the CLI; Node can't getUserMedia). Honest caveat: audio-publish is the hard half, needs a real browser to test, and Juke's own v1 API says agents cannot publish audio - only browser host/speaker can.
